...
public class Task35 {
    ...
    public static void main(String[] args) throws Exception {
        BufferedReader in1 = new BufferedReader(
                new FileReader("superText.txt", StandardCharsets.UTF_8));
        ...
        for (char c : ciphertext.toString().toCharArray()) {
            if (c == ' ') {
                ...
                char decrChar = (char) (Character.isUpperCase(c) ?
                        ('А' + (((c - 'А') - 3 + 33) % 33)) :
                        ('а' + (((c - 'а') - 3 + 33) % 33)));
                deText.append(decrChar);
            }
        }

        BufferedWriter out2 = new BufferedWriter(
                new FileWriter("shifr.txt", StandardCharsets.UTF_8, true));
        out2.write(deText.toString());
        out2.newLine();
        out2.close();

        System.out.println(deText.toString().replaceAll("\\s+", " "));
    }
}